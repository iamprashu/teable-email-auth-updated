  ...
  */
  ...
  async changeEmail(newEmail: string, password: string) {
    ...
    if (userByNewEmail && userByNewEmail.id !== userId) {
      throw new ConflictException('New email is already registered by another user');
    }
    ...
    await this.sessionStoreService.clearByUserId(userId);
    this.logger.log(`User ${userId} email changed from ${currentEmail} to ${newEmail} without verification.`);
  ...
  async sendChangeEmailCode(newEmail: string, password: string) {
    ...
    this.logger.warn(`sendChangeEmailCode called for ${newEmail} but email verification is bypassed. No verification email sent.`);
    const dummyCode = 'no_code_needed'; 
    const dummyToken = await this.jwtService.signAsync(
      { email, newEmail, code: dummyCode, bypassed: true },
      { expiresIn: '1m' } // A short expiry, as this token won't be used for actual verification
    );
